...
struct HudLayout {
    float x{}, y{}, width{};
    int lines{};
    static HudLayout fit(float width, float height, float horizontal, float vertical, int count) {
        if (!std::isfinite(width) || !std::isfinite(height) || width < 12 || height < 18 || count <= 0
            || !std::isfinite(horizontal) || !std::isfinite(vertical)) return {};
        float available = std::min(230.f,width-8);
